...
with open("./secret.json", "r") as file:
    secrets = load(file)
LLEDITSCRIPT = secrets["librelancer"] + "lleditscript"
...
def iconname(path):
    return splitext(PureWindowsPath(path).parts[-1])[0]
...
def get_systems(get_system_images=False) -> dict:
    ...
    if get_system_images:
        ...
        service = Service("/home/tim/.local/bin/geckodriver")

...
def position_camera(camera, target):
    ...
    bbox_size = max(
        (bbox_corners[i] - bbox_corners[j]).length
        for i in range(8)
    ...
    camera.location = bbox_center + Vector(
        (distance * math.cos(angle_45), distance * math.sin(angle_45), distance * 0.5)
    )
...
model_dir = "../dumpedData/models"
ships = {ship for ship in os.listdir(model_dir) if ship.endswith(".glb")}
start_time = time.perf_counter()
for i, ship_file in enumerate(ships):
    this_render = time.perf_counter()
    ship_name = ship_file.replace(".glb", "")
    # Replace 'path_to_your_file.gltf' and 'path_to_your_hdri.hdr' with the actual paths to your GLTF and HDRI files
    main(
        f"{model_dir}/{ship_file}",
        r"A:\Benutzer\Tim\Downloads\lonely_road_afternoon_puresky_4k.hdr",
    )
    ...
    im = Image.open(f"../dumpedData/images/ships/{ship_name}.png")
    im = im.crop(im.getbbox())
    # im.thumbnail(max_size)
    im.save(f"../dumpedData/images/ships/{ship_name}.png")
